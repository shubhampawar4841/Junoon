...
class NFDAScraper(BaseScraper):
    def __init__(self):
        super().__init__()
        self.base_url = "https://www.nfda.org/find-a-funeral-home"
    ...
    def scrape(self):
        ...
        for state in self.states:
            state_url = f"{self.base_url}?state={state}"
    ...
    def extract_listing_data(self, listing):
        try:
            name_elem = listing.find('h2', class_='funeral-home-name')
            ...
            type_elem = listing.find('div', class_='type')

            return {
                'Business Name': self.clean_text(name_elem.text) if name_elem else "N/A",
                'Type': self.clean_text(type_elem.text) if type_elem else "Funeral Home",
